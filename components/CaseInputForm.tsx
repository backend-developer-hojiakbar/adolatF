...
import { getDocumentType, analyzeLargeDocumentServer } from '../services/geminiService';
        ...
        await Promise.all(newPlaceholders.map(async (placeholder) => {
            ...
            try {
                ...
                if (file.type === 'application/pdf') {
                    // Use Backend for PDF (Large file support)
                    setFiles(prev => prev.map(f => f.id === placeholder.id ? { ...f, statusText: "Serverda tahlil qilinmoqda..." } : f));
                    extractedText = await analyzeLargeDocumentServer(file);
                    // Do NOT set content (base64) for large PDFs to avoid 400 Payload Too Large in later API calls
                } else {
                    // Local processing for other files
                    [content, extractedText] = await Promise.all([
                        readFileAsDataURL(file),
                        extractTextFromFile(file)
                    ]);
                }

                setFiles(prev => prev.map(f => f.id === placeholder.id ? { ...f, file, content, extractedText, statusText: t('file_status_analyzing') } : f));
