...
export const ParticipantEditorModal: React.FC<ParticipantEditorModalProps> = ({ initialParticipants, onConfirm, onCancel, isLoading, t }) => {
    ...
    const roleKeys = ["davogar", "javobgar", "sudlanuvchi", "jabrlanuvchi", "guvoh", "sudya", "boshqa"];

    // Mapping from Cyrillic role names (as returned by AI) to translation keys
    ...
    // Only initialize once to prevent user edits being lost on re-renders
    useEffect(() => {
        if (!initializedRef.current && initialParticipants.length > 0) {
            const mappedParticipants = initialParticipants.map((p, i) => {
                // Map AI suggested role (Cyrillic) to translation key, fallback to "boshqa"
                const roleKey = roleMapping[p.suggestedRole] || "boshqa";
                return {
                    id: `ai-${i}-${p.name}`,
                    name: p.name,
                    role: t(`client_role_${roleKey}`),
                    isAiSuggested: true,
                };
            });
    ...
    const isConfirmDisabled = isLoading || !clientId;
