  ...
  const handleFilesAdded = useCallback(async (fileList: File[]) => {
      ...
      const processSingleFile = async (file: File, placeholder: CaseFile): Promise<CaseFile> => {
        try {
          ...
          // Fix: Pass language argument and destructure the result of getDocumentType
          const { documentType } = await getDocumentType(fileWithData, t as any, language);
          const finalType = documentType === "Бошқа" ? t('file_status_ready') : documentType;
